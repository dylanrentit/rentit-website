...
export default function Community() {
  ...
  const MARQUEE_CLASS = prefersReducedMotion
    ? ""
    : "animate-[marquee_100s_linear_infinite] hover:[animation-play-state:paused]";
    ...
    <section id="community" className="section-pad">
      <div className="container-xy">
        ...
        <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-center">
          Join a community of renters
        </h2>
        <p className="text-slate-600 text-center mt-3 mb-8">
          2,000+ people (and counting) are lending weekly.
        </p>
        ...
        <div className="relative overflow-hidden">
          ...
          <div className={`flex w-max gap-6 ${MARQUEE_CLASS}`}>
            {[...PHOTOS, ...PHOTOS].map((src, i) => (
              <div
  key={i}
  className="relative shrink-0 snap-center rounded-3xl overflow-hidden ring-1 ring-black/10 shadow-card
             min-w-[280px] sm:min-w-[360px] md:min-w-[440px] aspect-[4/3] bg-slate-100"
>
  <img
    src={src}
  ...
  />
      ...
      <style>{`
        @keyframes marquee {
          ...
          100% { transform: translateX(-50%); } /* shift by one full set of cards */
        }
      `}</style>
    </section>
  );
